<template>
  <div ref="rootRef" v-if="showUsageWindows">
    ...
    <!-- OpenAI OAuth and OpenAI/Anthropic GLM Coding Plan API-key accounts use unified /usage. -->
    <template
      v-else-if="
        (account.platform === 'openai' && account.type === 'oauth') ||
        isGLMCodingPlanUsage
      "
    >
      ...
      <div v-else>
        ...
        <button
          v-else
          type="button"
          class="mt-1 inline-flex items-center gap-0.5 rounded px-1.5 py-0.5 text-[10px] font-medium text-blue-600 hover:bg-blue-50 dark:text-blue-400 dark:hover:bg-blue-900/30"
          :disabled="activeQueryLoading"
          @click="loadActiveUsage"
        >
          {{ t('admin.accounts.usageWindow.activeQuery') }}
...
const isGLMCodingPlanUsage = computed(() => {
  if (
    (props.account.platform !== 'openai' && props.account.platform !== 'anthropic') ||
    props.account.type !== 'apikey'
  ) return false
  return props.account.credentials?.glm_coding_plan_usage_enabled === true
})
...
const showUsageWindows = computed(() => {
  ...
  if (isGLMCodingPlanUsage.value) return true
...
const shouldFetchUsage = computed(() => {
  ...
  if (props.account.platform === 'openai') {
    return props.account.type === 'oauth' || isGLMCodingPlanUsage.value
  }
  return false
})
...
const hasOpenAIUsageFallback = computed(() => {
  if (!isGLMCodingPlanUsage.value && !(props.account.platform === 'openai' && props.account.type === 'oauth')) return false
