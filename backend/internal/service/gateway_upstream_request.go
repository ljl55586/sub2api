...
func (s *GatewayService) buildUpstreamRequestWithOptions(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID string, reqStream bool, mimicClaudeCode bool, opts upstreamRequestBuildOptions) (*http.Request, []byte, error) {
	...
	// OAuth mimic requests always finish with claude.DefaultHeaders below, so
	// their billing version must follow that final User-Agent rather than a
	// cached account fingerprint. Other paths retain the fingerprint behavior.
...
// 两条特殊规则：
//
//   - OAuth mimic：requiredBetas 为 ClaudeCodeOAuthCountTokensMimicryBetas + BetaTokenCounting
