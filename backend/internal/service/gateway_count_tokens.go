...
func (s *GatewayService) ForwardCountTokens(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) error {
	...
	// 发送请求
	resp, err := s.doCapturedClaudeHTTPUpstream(upstreamReq, proxyURL, account, s.tlsFPProfileService.ResolveTLSProfile(account))
	if err != nil {
	...
	if resp.StatusCode == 400 && s.shouldRectifySignatureError(ctx, account, respBody, reqModel) {
		...
		if buildErr == nil {
			retryResp, retryErr := s.doCapturedClaudeHTTPUpstream(retryReq, proxyURL, account, s.tlsFPProfileService.ResolveTLSProfile(account))
			if retryErr == nil {
